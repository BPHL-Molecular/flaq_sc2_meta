...
parser = argparse.ArgumentParser(usage='flaq_sc2_ww.py <input_dir> [options]')
...
c -B $(pwd):/data /apps/staphb-toolkit/containers/samtools_1.11.sif samtools view - -F 4 -u -h | singularity exec -B $(pwd):/data /apps/staphb-toolkit/containers/samtools_1.11.sif samtools sort > ' + align_dir + s + '.sorted.bam', shell=True, check=True)

    #If frag == 'frag', remove PCR duplicates
    elif frag == 'frag':
        subprocess.run('singularity exec -B $(pwd):/data /apps/staphb-toolkit/containers/bwa_0.7.17.sif bwa mem -t ' + threads + ' ' + ref + ' ' + sample_dir + s + '_1.fq.gz ' + sample_dir + s + '_2.fq.gz | singularity exec -B $(pwd):/data /apps/staphb-toolkit/containers/samtools_1.11.sif samtools view - -F 4 -u -h | singularity exec -B $(pwd):/data /apps/staphb-toolkit/containers/samtools_1.11.sif samtools sort -n > ' + align_dir + s + '.namesorted.bam', shell=True, check=True) #output name sorted bam
        subprocess.run('singularity exec -B $(pwd):/data /apps/staphb-toolkit/containers/samtools_1.11.sif samtools fixmate -m ' + align_dir + s + '.namesorted.bam ' + align_dir + s + '.fixmate.bam', shell=True, check=True) #fixmate
        #Create positional sorted bam from fixmate.bam
        subprocess.run('singularity exec -B $(pwd):/data /apps/staphb-toolkit/containers/samtools_1.11.sif samtools sort -o ' + align_dir + s + '.positionsort.bam ' + align_dir + s + '.fixmate.bam', shell=True, check=True)
        #Mark duplicate reads
        subprocess.run('singularity exec -B $(pwd):/data /apps/staphb-toolkit/containers/samtools_1.11.sif samtools markdup ' + align_dir + s + '.positionsort.bam ' + align_dir + s + '.markdup.bam', shell=True, check=True)
        #Remove duplicate reads
        subprocess.run('singularity exec -B $(pwd):/data /apps/staphb-toolkit/containers/samtools_1.11.sif samtools markdup -r ' + align_dir + s + '.positionsort.bam ' + align_dir + s + '.dedup.bam', shell=True, check=True)
        #Sort dedup.bam and rename to .sorted.bam
        subprocess.run('singularity exec -B $(pwd):/data /apps/staphb-toolkit/containers/samtools_1.11.sif samtools sort -o ' + align_dir + s + '.sorted.bam ' + align_dir + s + '.dedup.bam', shell=True, check=True)

    #Index final sorted bam from either no_frag or frag paths
    subprocess.run('singularity exec -B $(pwd):/data /apps/staphb-toolkit/containers/samtools_1.11.sif samtools index ' + align_dir + s + '.sorted.bam', shell=True, check=True)
    ...
    subprocess.run('singularity exec -B $(pwd):/data /apps/staphb-toolkit/containers/samtools_1.11.sif samtools sort ' + align_dir + s + '.primertrim.bam -o ' + align_dir + s + '.primertrim.sorted.bam', shell=True, stdout=out_log, stderr=err_log, check=True)
    subprocess.run('singularity exec -B $(pwd):/data /apps/staphb-toolkit/containers/samtools_1.11.sif samtools coverage ' + align_dir  + s + '.primertrim.sorted.bam -o ' + align_dir + s + '.coverage.txt', shell=True, stdout=out_log, stderr=err_log, check=True)
